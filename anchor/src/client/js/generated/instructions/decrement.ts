...
import { POKER_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof POKER_PROGRAM_ADDRESS,
  TAccountPoker extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountPoker extends string
        ? WritableAccount<TAccountPoker>
        : TAccountPoker,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountPoker extends string = string> = {
  poker: Address<TAccountPoker>;
};

export function getDecrementInstruction<
  TAccountPoker extends string,
  TProgramAddress extends Address = typeof POKER_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountPoker>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountPoker> {
  // Program address.
  const programAddress = config?.programAddress ?? POKER_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    poker: { value: input.poker ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.poker)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountPoker>;
